...
def get_web_root(wb, datadir_path, bitcoind_getinfo_var, stop_event=variable.Event(), static_dir=None):
    ...
    def get_patron_sendmany(total=None, trunc='0.01'):
        ...
        total = int(float(total)*1e7)
        trunc = int(float(trunc)*1e7)
        return json.dumps(dict(
            (bitcoin_data.script2_to_address(script, node.net.PARENT), value/1e7)
    ...
    def get_local_stats():
        ...
        return dict(
            ...
            block_value=node.bitcoind_work.value['subsidy']*1e-7,
    ...
    web_root.putChild('current_payouts', WebInterface(lambda: dict((bitcoin_data.script2_to_address(script, node.net.PARENT), value/1e7) for script, value in node.get_current_txouts().iteritems())))
    ...
    def update_stat_log():
        ...
        for add in wb.pubkeys.keys:
            my_current_payout+=node.get_current_txouts().get(bitcoin_data.pubkey_hash_to_script2(add), 0)*1e-7
        ...
        return dict(
            ...
            block=dict(
                ...
                gentx=dict(
                    ...
                    value=share.share_data['subsidy']*1e-7,
    ...
    def add_point():
        ...
        for add in wb.pubkeys.keys:
             my_current_payouts += current_txouts.get(bitcoin_data.pubkey_hash_to_script2(add), 0)*1e-7
        ...
        hd.datastreams['current_payouts'].add_datum(t, dict((user, current_txouts_by_address[user]*1e-7) for user in miner_hash_rates if user in current_txouts_by_address))
